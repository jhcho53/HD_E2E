...
class Planning(nn.Module):
    ...
    def forward(self,cam_front, trajs, gt_trajs, cost_volume, semantic_pred, hd_map, commands, target_points):
        ...
        # cam_front를 reduce_channel을 통해 차원 축소
        cam_front = self.reduce_channel(cam_front)  # (B, C, H, W)

        # Flatten 후 hidden_size=128로 변환
        h0 = cam_front.view(cam_front.size(0), -1)  # (B, flattened_size)

        # 선형 변환을 통해 256에서 128로 맞추기
        h0 = torch.nn.Linear(h0.size(1), 128).to(h0.device)(h0)  # 최종 (B, 128)

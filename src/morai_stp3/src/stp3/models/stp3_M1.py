...
class STP3(nn.Module):
    ...
    def encoder_forward(self, x, cam_front_index=1):
        """
        Forward pass through encoder with safety for cam_front_index and tensor shape adjustments.
        """
        # x 차원 확인 및 확장 (필요 시)
        if x.ndim == 4:  # 예상보다 낮은 차원일 경우
        ...
        # 안전한 cam_front_index 조정
        cam_front_index = min(cam_front_index, n - 1)

        # x를 (b * n, c, h, w) 형태로 인코딩
        x = x.view(b * n, c, h, w)  # 병합된 배치와 카메라 차원
        x, depth = self.encoder(x)  # Encoder 실행

        # Planning이 활성화된 경우 cam_front 추출
        if self.cfg.PLANNING.ENABLED:
            cam_front = x.view(b, n, *x.shape[1:])[:, cam_front_index]
        else:
            cam_front = None

        # Depth 분포 사용 여부에 따른 계산
        if self.cfg.MODEL.ENCODER.USE_DEPTH_DISTRIBUTION:
            depth_prob = depth.softmax(dim=1)
            x = depth_prob.unsqueeze(1) * x.unsqueeze(2)  # Depth와 Feature의 Outer Product
        else:
            x = x.unsqueeze(2).repeat(1, 1, self.depth_channels, 1, 1)

        # 최종 x 형태 조정
        x = x.view(b, n, *x.shape[1:])
        x = x.permute(0, 1, 3, 4, 5, 2)  # Channel을 마지막 차원으로 이동

        # Depth 차원 조정
        depth = depth.view(b, n, *depth.shape[1:])

        # 출력 형태 확인 (디버깅)
        print(f"Final x.shape: {x.shape}, depth.shape: {depth.shape}, cam_front.shape: {cam_front.shape if cam_front is not None else None}")
